...
// Problem: want to run the controller continuously, while the commands are not running
// so on each command, stop the controller, then start it again as part of the command
// solution: store the controller in state, have a future that runs in each command that checks for
// the next command
// so each command will signal that the next command should end

//struct CLIState {
//tor: TorInstance,
//controller: Option<StormController>,
//sender: UnboundedSender<OSSender<()>>,
//receiver: UnboundedReceiver<OSSender<()>>,
//user: Option<String>,
//}
...
////async fn take_control(&mut self) -> StormController {
////let (s_notify, r_notify) = futures::channel::oneshot::channel::<()>();
////self.sender.unbounded_send(s_notify);
////r_notify.await;
////self.controller.take().unwrap()
////}

//async fn run_controller(&mut self, controller: StormController) {
//loop {
//select!(
//os_sender = self.receiver.next()=> {
//self.controller.insert(controller);
//os_sender.send(());
//break;
//}
//output = controller.run_to_output().fuse() => {
...
// The shellfish library apparently won't work. We should:
// 1. Create controller
// 2. Capture stdout
// 3. in loop, do the following
//      - print stdout and show prompt
//      - accept input
//      - process the input
//      - process the controller while waiting for output
