...
pub struct StormController {
    ...
    pub tor_controller: Arc<Mutex<TorControl>>,
    key: KeyPair,
    connector: Connector,
...
impl StormController {
    ...
    pub fn send_group_message(&mut self, group: &str, body: &str) {
        let body = MessageBody {
            from: self.name.clone(),
            text: body.to_string(),
            dependencies: vec![],
        };
        self.send_message(get_group(group).id, body)
    }
        ...
        loop {
            select!(
                ...
                _ = self.sync.sync(false).fuse() => {
                    ...
                    return ControllerOutput::Exited;
                }
    ...
    pub fn clone_tor(&self) -> Rc<Mutex<TorInstance>> {
        self.tor.clone()
    }
...
impl Connector {
    pub fn new(name: String, key: KeyPair, tor: Arc<Mutex<TorControl>>) -> Self {
...
mod test {
    ...
    use crate::controller::tor::{TorInstance, TorSocket};
    ...
    #[tokio::test]
    async fn exchange_many_between_two() {
        // Test connection between 4 peers, several messages over 3 groups
        // connections:
        //      1-2,2-3,2-4
        // alternate groups of messages as so:
        ...
        let group = network.get_group("Group").id;
        let msg0 = message(None, "Blah", vec![]);
        let msg1 = message(None, "Blaa", vec![]);
        let msg2 = message(None, "Blar", vec![]);
        let msg3 = message(None, "Blae", vec![]);
        let msg4 = message(None, "Blaw", vec![]);
        let msg5 = message(None, "Blap", vec![]);
        let msg6 = message(None, "Blak", vec![]);
        let msg7 = message(None, "Blal", vec![]);
        let msg8 = message(None, "Bla;", vec![]);
        network.insert_message("Ashley", group, msg0);
        network.insert_message("Ashley", group, msg1);
        network.insert_message("Ashley", group, msg2);
        network.insert_message("Ashley", group, msg3);
        network.insert_message("Ashley", group, msg4);
        network.insert_message("Ashley", group, msg5);
        network.insert_message("Ashley", group, msg6);
        network.insert_message("Ashley", group, msg7);
        network.insert_message("Ashley", group, msg8);

        network.complete_connections();
        network.run_to_all_inactive();
        let msg_a = network.peek_msg("Ashley");
        network.assert_msg_fully_shared(&msg_a);
    }
}
